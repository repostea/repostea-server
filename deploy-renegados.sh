...
# 🚀 Renegados Server Deployment Script
# Deploys the Laravel server to production
#
# Usage:
#   ./deploy-renegados.sh              # Deploy with confirmation
#   ./deploy-renegados.sh --yes        # Deploy without confirmation
#   ./deploy-renegados.sh --migrations # Run migrations after deployment
...
print_status "🚀 Renegados Server Deployment"
echo ""

# Check if running as correct user (should be www-data for Laravel)
CURRENT_USER=$(whoami)
...
# Pull latest changes using shallow clone (more secure, no history)
...
# Install Composer dependencies (no dev dependencies in production)
...
# If DB_MEDIA_DATABASE is not set, add it with default value
if [ -z "$DB_MEDIA_DATABASE" ]; then
    print_warning "DB_MEDIA_DATABASE not found in .env, adding default value..."
    echo "" >> .env
    echo "# Media database (for images)" >> .env
    echo "DB_MEDIA_DATABASE=renegados_media" >> .env
    DB_MEDIA_DATABASE="renegados_media"
    print_success "✅ DB_MEDIA_DATABASE added to .env: renegados_media"
fi
...
if [ "$RUN_MIGRATIONS" = true ]; then
    ...
    if [ "$PENDING_MIGRATIONS" -gt 0 ]; then
        ...
        # Crear backup de la base de datos principal
        if command -v mysqldump &> /dev/null; then
            if mysqldump -h"${DB_HOST}" -P"${DB_PORT}" -u"${DB_USERNAME}" -p"${DB_PASSWORD}" "${DB_DATABASE}" > "${BACKUP_FILE}" 2>/dev/null; then
                ...
                print_success "✅ Backup principal creado: ${BACKUP_FILE}"
            else
                print_warning "⚠️  No se pudo crear backup de la base de datos principal"
            fi

            # Crear backup de la base de datos media si existe
            if [ -n "$DB_MEDIA_DATABASE" ]; then
                MEDIA_BACKUP_FILE="storage/backups/backup_media_$(date +%Y%m%d_%H%M%S).sql"
                if mysqldump -h"${DB_HOST}" -P"${DB_PORT}" -u"${DB_USERNAME}" -p"${DB_PASSWORD}" "${DB_MEDIA_DATABASE}" > "${MEDIA_BACKUP_FILE}" 2>/dev/null; then
                    ...
                    print_success "✅ Backup media creado: ${MEDIA_BACKUP_FILE}"
                else
                    print_warning "⚠️  No se pudo crear backup de la base de datos media (puede que no exista aún)"
                fi
            fi

            # Limpiar backups antiguos (mantener últimos 10 de cada tipo)
            ls -t storage/backups/backup_*.sql.gz 2>/dev/null | tail -n +11 | xargs -r rm
            ls -t storage/backups/backup_media_*.sql.gz 2>/dev/null | tail -n +11 | xargs -r rm
            print_success "✅ Backups antiguos limpiados (últimos 10 de cada tipo mantenidos)"
        else
            print_warning "⚠️  mysqldump no disponible, saltando backup"
        fi
    else
        echo ""
        print_status "========================================="
...
if command -v supervisorctl &> /dev/null; then
    # Queue workers
    if supervisorctl status repostea-worker:* &> /dev/null; then
        if supervisorctl restart repostea-worker:*; then
            print_success "✅ Queue workers restarted"
        else
            print_warning "⚠️  Failed to restart queue workers"
        fi
    else
        print_warning "⚠️  Queue workers not configured in supervisor"
        print_warning "Run: bash setup-queue-worker.sh to set them up"
    fi

    # Reverb WebSocket server
    if supervisorctl status repostea-reverb &> /dev/null; then
        if supervisorctl restart repostea-reverb; then
            print_success "✅ Reverb WebSocket server restarted"
        else
            print_warning "⚠️  Failed to restart Reverb"
        fi
    else
        print_warning "⚠️  Reverb not configured. Run: sudo ./setup-reverb.sh"
    fi
else
    print_warning "⚠️  Supervisor not installed"
    print_warning "Run: bash setup-queue-worker.sh to install and configure"
fi
...
echo ""
echo "🌐 API URL: https://api.renegados.es"
echo "🌐 Blade URL: https://repostea.renegados.es"
echo ""
...
echo "  Restart workers:     supervisorctl restart repostea-worker:*"
echo "  Worker status:       supervisorctl status repostea-worker:*"
